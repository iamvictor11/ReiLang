#ifndef C_TEMPLATE_CONTAINER_LIST_SINGLY_H
#define C_TEMPLATE_CONTAINER_LIST_SINGLY_H
...
#pragma region Dec
#define C_TEMPLATE_DECL_SLIST(ATTR, SPREFIX, LPREFIX, NAME, TYPE) \
...
#pragma region Def
#define C_TEMPLATE_DEFN_SLIST(ATTR, SPREFIX, LPREFIX, NAME, TYPE) \
...
#pragma region Impl
#define C_TEMPLATE_IMPL_SLIST(ATTR, SPREFIX, LPREFIX, NAME, TYPE, NODE_NAME, ALLOCATOR) \
    /* 私有 */ \
    ...
    { \
        ...
        TYPE* old = me->head; \
        me->head = SPREFIX##NAME##_GetNode(old)->next; \
    ...
    ATTR void SPREFIX##NAME##Erase(LPREFIX##NAME me, TYPE* element) \
    { \
        if (element == NULL || me->head == NULL) return; \
        if (element == me->head) { SPREFIX##NAME##PopFront(me); return; } \
        TYPE* prev = me->head; \
        while (prev != NULL && SPREFIX##NAME##_GetNode(prev)->next != element) \
            prev = SPREFIX##NAME##_GetNode(prev)->next; \
        if (prev == NULL) return; \
        TYPE* next = SPREFIX##NAME##_GetNode(element)->next; \
        SPREFIX##NAME##_GetNode(prev)->next = next; \
        if (element == me->tail) me->tail = prev; \
...
#pragma region Sugar
#define C_TEMPLATE_SLIST_FOREACH(TYPE, ELEM_PTR_NAME, NODE_NAME, CONTAINER_PTR) \
    for (TYPE* ELEM_PTR_NAME = (CONTAINER_PTR)->head; ELEM_PTR_NAME != NULL; ELEM_PTR_NAME = (ELEM_PTR_NAME)->NODE_NAME.next)
#pragma endregion
